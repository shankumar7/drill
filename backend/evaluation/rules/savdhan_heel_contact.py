...
class SavdhanHeelContactRule(EvaluationRule):
    ...
    def evaluate(self, detection: PoseDetection) -> RuleResult:
        ...
        # Heels should be touching. Since spine length is much longer than shoulder width,
        # we adjust the threshold. A gap of < 0.15 of spine length is considered touching.
        normalized_gap = gap / (scale + 1e-6)
        
        if normalized_gap <= 0.15:
            score = 100.0
        else:
            score = max(0.0, 100.0 - (normalized_gap - 0.15) * 500.0)
